...
void cancelUpload(){
    ...
    extern void WS_SEND(int args_ID, JSONVar& args_JSON);
    extern void WS_BROADCAST(JSONVar& args_JSON);
    extern void WS_PROC(JSONVar& args_JSON);
    extern JSONVar JSON_WS;

	void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {

        char STR_BUFF[128];
        sprintf(STR_BUFF, "WS_CLIENT_%03d", num);
        String HASH_WS_CLIENT = String(STR_BUFF);

		if (!JSON_WS.hasOwnProperty("INIT")) {
			JSON_WS = JSON.parse("{\"INIT\":true}");
		}

	    switch(type) {

	    case WStype_DISCONNECTED: {
		        JSON_WS[HASH_WS_CLIENT] = undefined;
	    	}
	        break;
	    case WStype_CONNECTED: {
		        IPAddress ip = socket_server->remoteIP(num);
		        JSONVar WS_CLIENT = JSON.parse("{}");
				WS_CLIENT["ID_KEY"] = HASH_WS_CLIENT;
		        WS_CLIENT["ID_FROM"] = num;
		        sprintf(STR_BUFF, "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
		        WS_CLIENT["IP_FROM"] = String(STR_BUFF);
		        sprintf(STR_BUFF, "%s", payload);
		        WS_CLIENT["URI"] = String(STR_BUFF);

		        JSON_WS[HASH_WS_CLIENT] = WS_CLIENT;

		        WS_SEND(num, WS_CLIENT);
		    }
		    break;
	    case WStype_TEXT: {
		        JSONVar JSON_ARGS  = JSON.parse((char*) payload);
		        if (JSON.typeof(JSON_ARGS) != "object") {
		        	JSON_ARGS = JSON.parse("{}");
					JSON_ARGS["TEXT"] = String((char*) payload);
		        }

				JSON_ARGS["ID_KEY"] = HASH_WS_CLIENT;
				JSON_ARGS["ID_FROM"] = num;

		        IPAddress ip = socket_server->remoteIP(num);
		        sprintf(STR_BUFF, "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
				JSON_ARGS["IP_FROM"] = String(STR_BUFF);;

		        JSONVar JSON_TEXT = JSON.parse("{}");
		        JSON_TEXT["ARGS"] = JSON_ARGS;
		        JSON_TEXT["RES"] = JSON.parse("{}");

		        WS_PROC(JSON_TEXT);

		        WS_SEND(num, JSON_TEXT);
		    }
	        break;
	    case WStype_BIN: {
		...
		//      socket_server->sendBIN(num, payload, length);
		    }
	        break;
	    default:
	        break;
	...
	//      USE_SERIAL.printf("[%u] Connected from %d.%d.%d.%d url: %s\n", num, ip[0], ip[1], ip[2], ip[3], payload);
	        ...
	        char STR_BUFF[128];
	        sprintf(STR_BUFF, "[%u] Connected from %d.%d.%d.%d url: %s\n", num, ip[0], ip[1], ip[2], ip[3], payload);

			String t = String(STR_BUFF);

	        socket_server->broadcastTXT(t);
	...
	//      USE_SERIAL.printf("[%u] get Text: %s\n", num, payload);
	        ...
	        // send message to client
	//      webSocket.sendTXT(num, "sendTXT message here");

	        // send data to all connected clients
	//      webSocket.broadcastTXT("broadcastTXT message here");
	        break;
	    case WStype_BIN:
	//      USE_SERIAL.printf("[%u] get binary length: %u\n", num, length);
	//      hexdump(payload, length);
