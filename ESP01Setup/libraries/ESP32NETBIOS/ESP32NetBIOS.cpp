 ...
 */
void ESP32NetBIOS::_getnbname(char *nbname, char *name, uint8_t maxlen)
{
    uint8_t b;
    uint8_t c = 0;
 ...
 */
void ESP32NetBIOS::_makenbname(char *name, char *nbname, uint8_t outlen)
{
    uint8_t b;
    uint8_t c = 0;
...
ESP32NetBIOS::ESP32NetBIOS():_pcb(NULL)
{
...
bool ESP32NetBIOS::begin(const char *name)
{
    ...
    addr.u_addr.ip4.addr = IPAddress(INADDR_ANY);
    _pcb = udp_new();
...
void ESP32NetBIOS::_recv(udp_pcb *upcb, pbuf *pb, const ip_addr_t *addr, uint16_t port)

{
    ...
    while(pb != NULL) {
        ...
        saddr.u_addr.ip4.addr = iphdr->src.addr;
...
void ESP32NetBIOS::_s_recv(void *arg, udp_pcb *upcb, pbuf *p, const ip_addr_t *addr, uint16_t port)
{
    reinterpret_cast<ESP32NetBIOS*>(arg)->_recv(upcb, p, addr, port);
}
